  ...
  render() {
    ...
    return (
      <form className="a-dashboard-page a-dashboard-payment">
        ...
        <ListItem price={this.props.prices.plusone} active={this.state.plusone} onClick={this.switchToPlusone}>
          <h3>Place visiteur</h3>
          <span>Réservé aux accompagnateurs.rices</span>
        </ListItem>
        <div className="a-dashboard-payment__separator"></div>
        <ListItem price={`+${this.props.prices.ethernet}`} active={this.state.ethernet} onClick={this.toggleEthernet}>
          <h3>Câble ethernet</h3>
          <span>Un câble (<strong>5m</strong>) est requis pour se brancher au switchs de tables</span>
        </ListItem>
        <ListItem price={`+${this.props.prices.shirt}`} active={this.state.shirt} onClick={this.toggleShirt}>
          <h3>T-Shirt UA 2018</h3>
          <span>Un t-shirt souvenir de cette LAN de folie</span>
          <div onClick={e => e.stopPropagation()}>
            <Select options={shirtGenders} defaultValue={this.state.shirtGender} onChange={this.changeGender} isSearchable={false} styles={selectStyles}></Select>
            <Select options={shirtSizes} defaultValue={this.state.shirtSize} onChange={this.changeSize} isSearchable={false} styles={selectStyles}></Select>
          </div>
        </ListItem>
        <div className="a-dashboard-payment__separator"></div>
